...
func TestRunCommandImportsLegacyCurrencySettingsWithVerifiedBackup(t *testing.T) {
	directory := t.TempDir()
	sourcePath := filepath.Join(directory, "legacy-currency-settings.db")
	...
	if _, err := source.Exec(`CREATE TABLE v2_settings(id INTEGER PRIMARY KEY,name TEXT,value TEXT);
		INSERT INTO v2_settings(name,value) VALUES ('currency','USD'),('currency_symbol','$')`); err != nil {
		t.Fatal(err)
	}
	_ = source.Close()
	...
	backupPath := filepath.Join(directory, "pre-currency-settings.xbbackup")
	now := time.Date(2026, 8, 30, 12, 30, 0, 0, time.UTC)
	var stdout, stderr bytes.Buffer
	handled, err := runCommand(t.Context(), []string{
		"migration", "import-legacy-currency-settings", "--source", sourcePath,
	...
	var result legacyCurrencySettingsMigrationCommandResult
	...
	_ = inspection.Close()
	if err != nil || settings.Currency != "USD" || settings.CurrencySymbol != "$" {
		t.Fatalf("migrated currency settings=%#v err=%v", settings, err)
	}

	stdout.Reset()
	handled, err = runCommand(t.Context(), []string{
		"migration", "import-legacy-currency-settings", "--source", sourcePath, "--confirm-offline",
...
func TestRunCommandRequiresOfflineConfirmationAndBackupForNewCurrencySettingsImport(t *testing.T) {
	directory := t.TempDir()
	sourcePath := filepath.Join(directory, "legacy-currency-settings.db")
	...
	t.Setenv("XBOARD_DATABASE_DSN", "file:"+targetPath)
	var stdout, stderr bytes.Buffer
	if handled, err := runCommand(t.Context(), []string{"migration", "import-legacy-currency-settings", "--source", sourcePath}, &stdout, &stderr, time.Now); !handled || err == nil || !strings.Contains(err.Error(), "confirm-offline") {
		t.Fatalf("offline gate handled=%t err=%v", handled, err)
	}
	if handled, err := runCommand(t.Context(), []string{"migration", "import-legacy-currency-settings", "--source", sourcePath, "--confirm-offline"}, &stdout, &stderr, time.Now); !handled || err == nil || !strings.Contains(err.Error(), "backup-output") {
