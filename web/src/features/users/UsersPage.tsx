...
type UsersAPI = Pick<AdminAPI,
  ...
  "deactivateAdminUser" | "restoreAdminUser" | "anonymizeAdminUser" | "getAdminUserLifecycleImpact"
>;
...
function UserLifecycleDialog({ api, account, action, onClose, onSaved }: {
  ...
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [impact, setImpact] = useState<UserLifecycleImpact | null>(null);
  const [impactLoading, setImpactLoading] = useState(action === "deactivate");
  const [impactError, setImpactError] = useState("");
  useEffect(() => {
    if (action !== "deactivate") return undefined;
    let active = true;
    void api.getAdminUserLifecycleImpact(account.id)
      .then((value) => { if (active) setImpact(value); })
      .catch((cause: unknown) => { if (active) setImpactError(cause instanceof Error ? cause.message : "无法读取停用影响，请刷新后重试"); })
      .finally(() => { if (active) setImpactLoading(false); });
    return () => { active = false; };
  }, [action, account.id, api]);
  ...
  return <Modal title={labels[action]} onClose={busy ? () => undefined : onClose}>
    ...
    <form onSubmit={(event) => void submit(event)}>
      ...
      {action === "deactivate" && <>
        <p>停用后立即撤销登录与订阅凭据，30 天内可恢复。订单、余额与佣金记录保留。</p>
        {impact && <div className="alert warning" role="status">
          <strong>停用影响预览</strong>
          <div>订单 {impact.orders} 条，邀请码 {impact.invitation_codes} 个，佣金记录 {impact.commission_logs} 条，提现 {impact.withdrawals} 笔，工单 {impact.tickets} 个（消息 {impact.ticket_messages} 条）。</div>
          <div>分销关系 {impact.distributor_relations} 条，附件 {impact.attachments} 个；账户余额 {formatMoney(impact.balance)}，佣金余额 {formatMoney(impact.commission_balance)}。</div>
          <div className="muted">预览版本 {impact.revision}，执行时服务端会重新校验最新状态。</div>
        </div>}
        {impactLoading && <p role="status">正在读取停用影响…</p>}
      ...
      <div className="form-actions"><button className="button ghost" type="button" disabled={busy} onClick={onClose}>取消</button><button className="button danger" type="submit" disabled={busy || !confirmed || (action === "deactivate" && (impactLoading || impact === null || impactError !== ""))}>{busy ? "处理中…" : "确认执行"}</button></div>
    </form>
  </Modal>;
}
