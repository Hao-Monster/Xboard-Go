...
export type SystemConfigShellAPI = Parameters<typeof SiteSettingsPage>[0]["api"] &
  Parameters<typeof SubscriptionSettingsPage>[0]["api"] &
  Parameters<typeof CommissionSettingsPage>[0]["api"] &
  Parameters<typeof NodeAgentSettingsPage>[0]["api"] &
  Parameters<typeof EmailSettingsPage>[0]["api"] &
  Parameters<typeof TelegramSettingsPage>[0]["api"] &
  Parameters<typeof ClientAppSettingsPage>[0]["api"];
...
export function SystemConfigShell({
  ...
  const [internalTab, setInternalTab] = useState<SystemConfigTab>(activeTab);
  const tab = onTabChange !== undefined ? activeTab : internalTab;

  const handleSelectTab = (nextTab: SystemConfigTab) => {
    if (nextTab === tab || onBeforeTabChange?.() === false) return;
