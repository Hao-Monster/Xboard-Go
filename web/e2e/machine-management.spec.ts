...
import { adminEmail, adminPassword, adminAPIPath, adminEntryPath } from "./support";
...
test("[FE-MACH-001][API-MACH-002][FE-MACH-003][SYS-MACH-004] machine lifecycle stays atomic and recoverable", async ({ page }) => {
  ...
  await page.unroute(`**${adminAPIPath(`/api/v1/admin/machines/${machineID}`)}`);
  ...
  const deleteResponse = page.waitForResponse((response) =>
    response.request().method() === "DELETE" && new URL(response.url()).pathname === adminAPIPath(`/api/v1/admin/machines/${machineID}`)
  );
  await deleteDialog.getByRole("button", { name: "确认删除" }).click();
