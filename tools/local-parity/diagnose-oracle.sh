...
run_abs="$root/$run"; [[ "$run_abs" == "$root/.local/local-parity-"* && "$run_abs" != *..* ]] || { echo 'unsafe run directory' >&2; exit 2; }
mkdir -p "$out"; exec > >(tee "$out/driver.log") 2>&1
cleanup() { rc=$?; printf 'driver_exit=%s\n' "$rc" >"$out/exit.txt"; docker compose -p "$project" -f compose.local.yaml -f tools/local-parity/compose.user-parity.yaml --profile e2e down --volumes --remove-orphans >>"$out/cleanup.log" 2>&1 || true; rm -rf -- "$run_abs"; exit "$rc"; }; trap cleanup EXIT
...
for n in 1 2 3; do
  { printf 'attempt=%s time=%s\n' "$n" "$(date -u +%FT%TZ)"; "${c[@]}" exec -T legacy-oracle supervisorctl status || true; "${c[@]}" exec -T legacy-oracle sh -c 'ps -o pid,stat,comm; ss -ltn 2>/dev/null || netstat -ltn 2>/dev/null || true'; printf 'outer='; curl -sS -m 5 -o /dev/null -w '%{exitcode},%{http_code},%{size_download}\n' "http://127.0.0.1:$port/" || true; printf 'inner='; "${c[@]}" exec -T legacy-oracle sh -c "curl -sS -m 5 -o /dev/null -w '%{exitcode},%{http_code},%{size_download}\\n' http://127.0.0.1:7002/" || true; } >>"$out/probe.txt" 2>&1
