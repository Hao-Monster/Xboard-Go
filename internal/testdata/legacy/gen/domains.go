...
VALUES
    (201, 'en-US', 'Getting Started', 'Synthetic knowledge article',
     'Synthetic knowledge body for migration verification.', NULL, 1, 1700000000, 1700000100);
...
INSERT INTO v2_gift_card_template VALUES
    (121, 'Synthetic balance card', 'Synthetic gift card template', 1, 1, '{}', '{"balance":500}', '{"max_use_per_user":2}', '{}', NULL, NULL, '#1890ff', 1, 1, 1700000000, 1700000100);
INSERT INTO v2_gift_card_code VALUES
    (131, 121, 'SYNTHETICGC000131', 'synthetic_batch_0131', 1, 2, 1700000200, NULL, '{"balance":500}', 1, 2, '{}', 1700000100, 1700000200);
INSERT INTO v2_gift_card_usage VALUES
    (141, 131, 121, 2, NULL, '{"balance":500}', NULL, 9, 11, 1.0, '198.51.100.10', 'Synthetic migration agent', 'Synthetic redemption', 1700000200);
...
func countRepresentativeDomains(ctx context.Context, tx *sql.Tx) (map[string]int, error) {
	domains := []struct {
		...
		{"coupons", "v2_coupon"},
		{"gift_card_templates", "v2_gift_card_template"},
		{"gift_card_codes", "v2_gift_card_code"},
		{"gift_card_usages", "v2_gift_card_usage"},
		{"payments", "v2_payment"},
		{"orders", "v2_order"},
		{"tickets", "v2_ticket"},
