...
func TestGeneratorProducesDeterministicDataset(t *testing.T) {
	const expectedSHA256 = "e1830fc3ade87bf17de87c8cf4e2c0138b598f0bd98a413f2c73f18307074f78"
	dir := t.TempDir()
...
func TestGeneratorProducesRepresentativeMigrationDomains(t *testing.T) {
	...
	for _, table := range []string{
		...
		"v2_knowledge", "v2_knowledge_attachment", "v2_knowledge_attachment_upload",
		"personal_access_tokens", "v2_invite_code", "v2_coupon",
		"v2_gift_card_template", "v2_gift_card_code", "v2_gift_card_usage",
		"v2_payment", "v2_order",
	...
	for _, table := range []string{"v2_knowledge_attachment", "v2_knowledge_attachment_upload"} {
		var rows int
		if err := database.QueryRow(`SELECT COUNT(*) FROM "` + table + `"`).Scan(&rows); err != nil {
			t.Fatalf("count empty representative table %q: %v", table, err)
		}
		if rows != 0 {
			t.Errorf("representative table %q rows = %d, want 0 before attachment-file migration", table, rows)
		}
	}
...
func TestGeneratedDatasetSatisfiesImplementedMigrationReaders(t *testing.T) {
	...
	readers := []struct {
		...
		{"knowledge", func(ctx context.Context, path string) error {
			_, err := legacymigration.ReadKnowledgeSnapshot(ctx, path)
			return err
		}},
		...
		{"gift-cards", func(ctx context.Context, path string) error {
			_, err := legacymigration.ReadGiftCardsSnapshot(ctx, path)
			return err
		}},
