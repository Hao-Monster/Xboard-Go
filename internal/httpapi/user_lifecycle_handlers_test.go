...
func TestUserLifecycleHTTPPermissionsSessionsAndLegacyDestroy(t *testing.T) {
	...
	user := payload.Data
	impact := admin.request(t, api, http.MethodGet, fmt.Sprintf("/api/v1/admin/admin/users/%d/lifecycle-impact", user.ID), "")
	if impact.Code != http.StatusOK || !strings.Contains(impact.Body.String(), `"lifecycle_status":"active"`) || !strings.Contains(impact.Body.String(), `"revision":`) {
		t.Fatalf("lifecycle impact %d %s", impact.Code, impact.Body)
	}
