...
func TestCommissionSettingsModernAndLegacyContractsAreStrictAndAudited(t *testing.T) {
	...
	if initial.Revision != 1 || initial.InviteCommission != 10 || !initial.FirstTimeEnabled ||
		...
		initial.WithdrawLimit != 10_000 || strings.Join(initial.WithdrawMethods, ",") != "支付宝,USDT,Paypal" ||
	...
	if updated.Revision != 2 || updated.InviteCommission != 25 || updated.FirstTimeEnabled || updated.AutoCheckEnabled ||
		updated.WithdrawLimit != 12_525 || strings.Join(updated.WithdrawMethods, ",") != "USDT,Paypal" ||
	...
	unknown := administrator.request(t, api, http.MethodPut, "/api/v1/admin/admin/commission-settings", `{
		...
		"commission_auto_check_enable":true,"withdraw_close_enable":false,
		"commission_distribution_enable":false,"commission_distribution_l1":100,
		"commission_distribution_l2":0,"commission_distribution_l3":0,"unexpected":true
	}`)
	...
	if legacyFetch.Code != http.StatusOK || !containsAll(legacyFetch.Body.String(),
		...
		`"commission_withdraw_limit":125.25`, `"commission_withdraw_method":["USDT","Paypal"]`,
		`"invite_force":false`, `"invite_gen_limit":5`, `"invite_never_expire":false`) {
		t.Fatalf("legacy invite fetch status=%d body=%s", legacyFetch.Code, legacyFetch.Body)
	}
	if strings.Contains(legacyFetch.Body.String(), "revision") {
		t.Fatalf("legacy invite fetch disclosed internal fields: %s", legacyFetch.Body)
	}
	legacySite := bearerRequest(api, http.MethodGet, "/api/v2/admin/config/fetch?key=site", legacyAuthorization, "")
	if legacySite.Code != http.StatusOK || !containsAll(legacySite.Body.String(), `"currency":"CNY"`, `"currency_symbol":"¥"`, `"force_https":0`, `"subscribe_url":""`) {
	...
	legacyDecimalFetch := bearerRequest(api, http.MethodGet, "/api/v2/admin/config/fetch?key=invite", legacyAuthorization, "")
	if legacyDecimalFetch.Code != http.StatusOK || !strings.Contains(legacyDecimalFetch.Body.String(), `"commission_withdraw_limit":250.5`) {
		t.Fatalf("legacy decimal withdrawal limit status=%d body=%s", legacyDecimalFetch.Code, legacyDecimalFetch.Body)
	}
	legacyUnknown := bearerRequest(api, http.MethodPost, "/api/v2/admin/config/save", legacyAuthorization, `{
		"invite_commission":20,"commission_first_time_enable":true,
		"commission_auto_check_enable":true,"withdraw_close_enable":false,
		"commission_distribution_enable":true,"commission_distribution_l1":50,
		"commission_distribution_l2":30,"commission_distribution_l3":20,"commission_withdraw_currency":"USD"
	}`)
	...
	if err != nil || preserved.Revision != 4 || preserved.InviteCommission != 20 || !preserved.DistributionEnabled ||
		preserved.WithdrawLimit != 25_050 || strings.Join(preserved.WithdrawMethods, ",") != "USDT" {
