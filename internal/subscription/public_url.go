...
func BuildPublicURL(config PublicURLConfig, token, fragment string) (string, error) {
	...
	if !validPublicSubscriptionPath(path) || !validPublicSubscriptionToken(token) {
	...
	if err != nil || parsed.Scheme == "" || parsed.Host == "" || parsed.Hostname() == "" || parsed.User != nil || parsed.Opaque != "" || !validPublicOriginPort(parsed.Port()) {
...
func validPublicSubscriptionPath(value string) bool {
...
func validPublicSubscriptionToken(value string) bool {
	if len(value) != 32 {
		return false
	}
	for _, character := range value {
