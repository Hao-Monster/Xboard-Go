...
// UserLifecycleImpact is the read-only confirmation payload shown before a
// destructive lifecycle request. Counts are recomputed by the mutation
// transaction; this snapshot is informational and never authorizes a change.
type UserLifecycleImpact struct {
	...
	DistributorRelations int64  `json:"distributor_relations"`
	Attachments          int64  `json:"attachments"`
	Balance              int64  `json:"balance"`
	CommissionBalance    int64  `json:"commission_balance"`
}

func (s *Store) GetUserLifecycleImpact(ctx context.Context, userID int64) (UserLifecycleImpact, error) {
	if userID < 1 {
		return UserLifecycleImpact{}, ErrInvalidInput
	}
	row := s.db.QueryRowContext(ctx, `SELECT id,admin_revision,balance,commission_balance FROM users WHERE id=? AND account_kind='human'`, userID)
	var result UserLifecycleImpact
	if err := row.Scan(&result.UserID, &result.Revision, &result.Balance, &result.CommissionBalance); err != nil {
		if err == sql.ErrNoRows {
			return UserLifecycleImpact{}, ErrNotFound
		}
		return UserLifecycleImpact{}, fmt.Errorf("read lifecycle impact user: %w", err)
	}
	if err := s.db.QueryRowContext(ctx, `SELECT CASE WHEN anonymized_at IS NOT NULL THEN 'anonymized' WHEN deactivated_at IS NOT NULL THEN 'deactivated' ELSE 'active' END FROM user_lifecycles WHERE user_id=?`, userID).Scan(&result.LifecycleStatus); err != nil {
		if err == sql.ErrNoRows {
			result.LifecycleStatus = "active"
		} else {
			return UserLifecycleImpact{}, fmt.Errorf("read lifecycle impact state: %w", err)
		}
	}
	queries := []struct {
		dst   *int64
		...
		{&result.Orders, `SELECT COUNT(*) FROM orders WHERE user_id=?`},
		{&result.InvitationCodes, `SELECT COUNT(*) FROM invitation_codes WHERE user_id=?`},
		{&result.CommissionLogs, `SELECT COUNT(*) FROM commission_logs WHERE user_id=? OR invite_user_id=?`},
		{&result.Withdrawals, `SELECT COUNT(*) FROM commission_withdrawals WHERE user_id=?`},
		{&result.Tickets, `SELECT COUNT(*) FROM tickets WHERE user_id=?`},
		{&result.TicketMessages, `SELECT COUNT(*) FROM ticket_messages WHERE user_id=?`},
		{&result.DistributorRelations, `SELECT COUNT(*) FROM distributor_subscriptions WHERE distributor_user_id=? OR subscriber_user_id=?`},
		{&result.Attachments, `SELECT COUNT(*) FROM knowledge_attachments WHERE uploader_user_id=?`},
	}
	for _, item := range queries {
		args := []any{userID}
		if strings.Count(item.query, "?") == 2 {
			args = append(args, userID)
		}
		if err := s.db.QueryRowContext(ctx, item.query, args...).Scan(item.dst); err != nil {
			return UserLifecycleImpact{}, fmt.Errorf("read lifecycle impact: %w", err)
		}
	}
	return result, nil
}
