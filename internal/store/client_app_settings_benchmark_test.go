...
func BenchmarkLegacyClientAppConfigLookup100KUsers(b *testing.B) {
	database := openBenchmarkStore(b, "client-app-config-100k.db")
	ctx := b.Context()
	if _, err := database.db.ExecContext(ctx, `
		WITH RECURSIVE sequence(value) AS (
			SELECT 1 UNION ALL SELECT value + 1 FROM sequence WHERE value < 100000
		...
		INSERT INTO users (email,password_hash,subscription_token,created_at,updated_at)
		SELECT printf('client-config-%06d@example.test',value),'hash',printf('%032x',value),0,0
		FROM sequence
	`); err != nil {
		b.Fatal(err)
	}
	...
	b.ResetTimer()
	for b.Loop() {
		if _, err := database.FindSubscriptionAccount(ctx, token); err != nil {
			b.Fatal(err)
		}
