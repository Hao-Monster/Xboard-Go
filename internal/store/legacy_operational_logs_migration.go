...
func scanOperationalFactStatistics(rows *sql.Rows, location *time.Location) ([]operationalStatistic, error) {
	...
	days := map[statisticKey]operationalStatistic{}
	for rows.Next() {
		...
		key := statisticKey{stamp: stamp, metric: value.Metric}
		day := days[key]
	...
	sort.Slice(values, func(i, j int) bool {
		if values[i].RecordAt == values[j].RecordAt {
			return values[i].Metric < values[j].Metric
		}
		return values[i].RecordAt < values[j].RecordAt
	})
	return values, nil
}
