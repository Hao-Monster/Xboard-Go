...
func TestSchemaV59GuardsCombinedUserTrafficWithoutLosingBoundaryRows(t *testing.T) {
	...
	if version != int64(CurrentSchemaVersion()) || upload != maximum-1 || download != 1 {
		t.Fatalf("schema/user traffic = %d/%d/%d, want current schema version/%d/1", version, upload, download, maximum-1)
	}
