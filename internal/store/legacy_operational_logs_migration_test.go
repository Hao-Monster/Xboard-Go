...
func TestScanOperationalFactStatisticsKeepsMetricsSeparateOnSameDay(t *testing.T) {
	database := newTestStore(t)
	rows, err := database.db.Query(`
		SELECT 1700000000 AS record_at, 'order_count' AS metric, 3 AS value
		UNION ALL SELECT 1700003600, 'order_total', 900
		UNION ALL SELECT 1700007200, 'order_count', 2
	`)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(values) != 2 {
		t.Fatalf("same-day metric rows = %#v, want two independent metrics", values)
	}
	if values[0].Metric != "order_count" || values[0].Value != 5 || values[1].Metric != "order_total" || values[1].Value != 900 {
		t.Fatalf("same-day metric aggregation = %#v", values)
	}
}
