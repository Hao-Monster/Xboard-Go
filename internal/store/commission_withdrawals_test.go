...
func TestCommissionWithdrawalMigrationDoesNotSettleHistoricalTicket(t *testing.T) {
	...
	if _, err := db.db.Exec(`DROP TRIGGER users_money_admin_revision; DROP TABLE commission_withdrawal_events; DROP TABLE commission_withdrawals; DROP TABLE legacy_operational_logs; DROP TABLE operational_daily_statistics; PRAGMA user_version = 60`); err != nil {
		t.Fatal(err)
	}
