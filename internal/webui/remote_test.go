...
func TestRemoteHandlerServesOnlyAuthorizedEntriesAndDelegatesBackend(t *testing.T) {
	type observedRequest struct {
		method, path, rawQuery, cookie, forwardedFor string
	}
	...
	frontend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		observed = append(observed, observedRequest{
			...
			cookie: r.Header.Get("Cookie"), forwardedFor: r.Header.Get("X-Forwarded-For"),
		})
		observedMu.Unlock()
	...
	for _, request := range observed {
		if request.method != http.MethodGet || request.path != "/index.html" || request.rawQuery != "" || request.cookie != "" || request.forwardedFor != "" {
