...
const (
	remoteIndexMaxBytes          = 2 << 20
	remoteFrontendMaxConnections = 64
	remoteFrontendRequestTimeout = 5 * time.Second
)
...
func NewRemote(origin string, api http.Handler, accessResolvers ...FrontendAccessResolver) (http.Handler, error) {
	...
	directTransport.MaxIdleConns = remoteFrontendMaxConnections
	directTransport.MaxIdleConnsPerHost = remoteFrontendMaxConnections
	directTransport.MaxConnsPerHost = remoteFrontendMaxConnections
	directTransport.ResponseHeaderTimeout = 4 * time.Second
	client := &http.Client{
		Transport: directTransport,
		Timeout:   remoteFrontendRequestTimeout,
